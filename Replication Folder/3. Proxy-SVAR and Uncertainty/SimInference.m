...
% ------------------------------------------------------------------------------
    ...
    %% 3)Critical Values for Rectangular Bands:
        ...
        function [Pwise,... %Sidak,Bonferroni,
                thetaproj,muproj]...
 ...
 %{        
            k       = size(Sigma,1);
sig       = sqrt(diag(Sigma));        % [σ₁; …; σₖ]
sig(end)  = 1;                        % force last to 1
D       = diag(1./sig);               % only divides by σ for vars 1…k−1
% now normalize all but leave last unchanged
Corrmat = D * Sigma * D;  
%}
        ...
        function the_bands = bands_plugin(thetahat, Vartheta, p, band_list, I, confidence)
            ...
            [cv.Pwise,...
             ...
             cv.muproj]...
                 ...
                 %I removed cv.Sidak,...
                 %          cv.Bonferroni,...
                 %as the second and third output of the function!
            for j    = 1:numbands
